...
// Define the function to fetch category ID by category name
function fetchCategoryIdByCategoryName(categoryName, callback) {
  ...
  db.query(query, [categoryName], (err, results) => {
    if (err) {
      ...
      if (results.length > 0) {
        ...
        callback(null, null); // Category not found
      }
    }
  });
}
