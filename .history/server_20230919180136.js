...
// Route to fetch products by disponibilite status
app.get('/products/disponibilite/:status', async (req, res) => {
  const status = req.params.status; // Should be either "Disponible" or "non_disponible"

  try {
    // Fetch products based on disponibilite status
    const [products] = await db.query(
      "SELECT * FROM products WHERE disponibilite = ?",
      [status]
    );

    if (!products || products.length === 0) {
      return res.status(404).json({ message: 'No products found with the specified disponibilite status.' });
    }
