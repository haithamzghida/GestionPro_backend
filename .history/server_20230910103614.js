...
app.get('/products/:categoryName', async (req, res) => {
  ...
  try {
    const categoryId = await connection.query("SELECT id FROM categories WHERE name = ?", [categoryName]);

    if (!categoryId || categoryId.length === 0) {
      return res.status(404).json({ message: 'Category not found.' });
    }

    // Assuming categoryId is an integer, convert it to a string before using it as a parameter
    const categoryIdStr = categoryId[0].id.toString();
