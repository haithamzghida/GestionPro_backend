...
app.put('/products/:productId/disponibilite', (req, res) => {
  ...
  // Ensure that the "disponibilite" field is provided
  if (!disponibilite) {
    return res.status(400).json({ error: 'The "disponibilite" field is required' });
  }
  ...
  connection.query(query, [disponibilite, productId], (error, results) => {
    if (error) {
