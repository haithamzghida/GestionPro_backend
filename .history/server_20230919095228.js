...
// Route to get all fournisseur names
app.get('/fournisseurs/names', (req, res) => {
  connection.query('SELECT nom FROM fournisseur', (err, rows) => {
    if (err) {
      console.log('Error fetching fournisseur names:', err);
      res.status(500).json({ error: 'Error fetching fournisseur names' });
    } else {
      const fournisseurNames = rows.map((row) => row.nom);
      res.json({ fournisseurNames });
    }
  });
});
