...
app.post('/login_inventory', (req, res) => {
  ...
  connection.query(query, (error, results) => {
    ...
    if (results.length === 1) {
      ...
      const userRoleQuery = 'SELECT role FROM login_inventory WHERE email = ?';
      connection.query(userRoleQuery, [email], (err, rows) => {
        if (err) {
          ...
          if (rows.length === 0) {
            ...
            // User found, send the user role along with the success response
