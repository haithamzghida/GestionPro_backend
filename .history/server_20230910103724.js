...
app.get('/products/:categoryName', async (req, res) => {
  ...
  try {
    ...
    // Check if the 'id' property exists in the categoryRecord object before accessing it
    if (!categoryRecord.hasOwnProperty('id')) {
      return res.status(500).json({ message: 'Category record is missing the id property.' });
    }

    const categoryIdStr = categoryRecord.id.toString();

    const [products] = await connection.query("SELECT * FROM products WHERE category_id = ?", [categoryIdStr]);
