...
app.get('/products/:categoryName', async (req, res) => {
  ...
  try {
    ...
    if (!category || category.length === 0 || category[0].id === undefined) {
      return res.status(404).json({ message: 'Category not found.' });
    }

    const categoryIdStr = category[0].id.toString();

    console.log("Category ID:", categoryIdStr); // Add this line for debugging
