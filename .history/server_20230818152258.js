...
app.get('/products/:categoryName', async (req, res) => {
  ...
  try {
    ...
    const category = await yourDatabaseQueryToFetchCategoryIdByCategoryName(categoryName);
    
    if (!category) {
      return res.status(404).json({ message: 'Category not found.' });
    }

    // Fetch products from the database based on the fetched category ID
    const products = await yourDatabaseQueryToFetchProductsByCategoryId(category.id);
